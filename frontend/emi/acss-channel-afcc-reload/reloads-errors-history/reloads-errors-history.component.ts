...
@Component({
  ...
  selector: 'afcc-reload-error-history',
  templateUrl: './reloads-errors-history.component.html',
  styleUrls: ['./reloads-errors-history.component.scss'],
  animations: fuseAnimations
})
export class ReloadErrorsHistoryComponent implements OnInit, OnDestroy {
  // Rxjs subscriptions
  subscriptions = [];

  dataSource = new MatTableDataSource();
  displayedColumns = ['date', 'name', 'buName', 'sourceIp', 'amount'];
  ...
  ngOnInit() {
    ...
    this.subscriptions.push(
      this.acssChannelAfccReloadService.fetchTotalReloadErrorsCount$()
      .pipe(
  ...
  refreshDataTable(page, count, searchFilter) {
    this.acssChannelAfccReloadService.getBasicReloadErrorsInfo$(page, count, searchFilter)
    .pipe(
      mergeMap(resp => this.graphQlErrorHandler$(resp)),
      filter((resp: any) => !resp.errors || resp.errors.length === 0),
